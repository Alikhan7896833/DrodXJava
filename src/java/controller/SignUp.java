    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (user_DTO.getFirst_name().isEmpty()) {
            ...
            response_DTO.setContent("Password must include at least one uppercase letter,number,"
                    + "special character and at be least 8 characters long");
        } else {
            ...
            if (!criteria1.list().isEmpty()) {
                response_DTO.setContent("User with thiis Email already exists");
            } else {
                ...
                Thread sendMailThread = new Thread() {
                    @Override
                    public void run() {
                        Mail.sendMail((user_DTO.getEmail()), "DrodX Store Verification",
                                "<h1 style=\"color:#64482AD;\">Your Verificatin Code: " + user.getVerification() + "</h1>"
                        );
                    }
